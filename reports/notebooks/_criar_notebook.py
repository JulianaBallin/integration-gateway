...
cells.append(new_code_cell("\n".join([
    ...
    "        self._lines = []",
    "",
    "    def _wrap_lines(self, avW):",
    "        from reportlab.pdfbase.pdfmetrics import stringWidth",
    "        max_w = max(avW - 2 * self._pad, 20)",
    "        wrapped = []",
    ...
    "                candidate = (current + (' ' if current.strip() else '') + word).rstrip()",
    "                if current.strip() and stringWidth(candidate, 'Courier', self._fs) > max_w:",
    ...
    "    def wrap(self, avW, avH):",
    "        self.width  = avW",
    "        self._lines = self._wrap_lines(avW)",
    "        lh = self._fs * 1.45",
    "        self.height = len(self._lines) * lh + 2 * self._pad",
    "        return self.width, self.height",
    "",
    "    def draw(self):",
    "        c = self.canv",
...
cells.append(new_code_cell("\n".join([
    ...
    "    ], [0.25, 0.30, 0.45]))",
    "    e.append(H3('8.1 Verificação via docker inspect'))",
    "    e.append(CodeBlock(",
    "        'docker inspect integration-gateway-transformer-1 --format \"User={{.Config.User}} Memory={{.HostConfig.Memory}} NanoCpus={{.HostConfig.NanoCpus}} ReadonlyRootfs={{.HostConfig.ReadonlyRootfs}} CapDrop={{json .HostConfig.CapDrop}} SecurityOpt={{json .HostConfig.SecurityOpt}}\"\\n'",
    "        '# Deve retornar: user não-root, memória/cpu definidas, read_only=true, CapDrop=ALL, no-new-privileges'",
    "    ))",
    "    return e",
    "",
    "print('Seções 6-8 OK')",
...
cells.append(new_code_cell("\n".join([
    ...
    "         'docker compose exec gateway-api python -c \"import urllib.request; print(urllib.request.urlopen(\\\'http://localhost:8000/health\\\').read().decode())\"\\n'",
    "         'docker compose exec audit-api python -c \"import urllib.request; print(urllib.request.urlopen(\\\'http://localhost:8300/health\\\').read().decode())\"',",
    ...
    "         'docker compose exec audit-api python -c \"import urllib.request; print(urllib.request.urlopen(\\\'http://localhost:8300/audits/summary\\\').read().decode())\"\\n'",
    "         '# SUCCESS: 3 | stable-v1',",
    "         'ev05', '/audits/summary no fluxo normal', 5*cm),",
    "        ('9.6 Validar hardening com inspect',",
    "         'docker inspect integration-gateway-transformer-1 --format \"User={{.Config.User}} Memory={{.HostConfig.Memory}} NanoCpus={{.HostConfig.NanoCpus}} ReadonlyRootfs={{.HostConfig.ReadonlyRootfs}} CapDrop={{json .HostConfig.CapDrop}} SecurityOpt={{json .HostConfig.SecurityOpt}}\"',",
    "         'ev06', 'docker inspect confirmando hardening', 5*cm),",
    "    ]",
